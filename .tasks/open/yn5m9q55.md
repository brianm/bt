---
title: Auto-install yatl-tasks skill on init when Claude Code detected
id: yn5m9q55
...
On `yatl init`, detect if Claude Code is in use and auto-install the yatl-tasks skill.
...
```rust
const SKILL_MD: &str = include_str!("../claude/yatl-tasks/SKILL.md");
const CLI_REFERENCE_MD: &str = include_str!("../claude/yatl-tasks/references/CLI_REFERENCE.md");
// etc.
```
...
Add `yatl install-skill` command that copies from a known location.

Pros: Explicit, user-controlled
Cons: Extra step for users

## Recommended: Option A (embedded)
Embed skill files in binary. On `yatl init`:
1. Check for Claude Code indicators
2. If found, create `.claude/commands/yatl-tasks/` in project
...
- Overwrite existing skill files on init (yatl init doubles as upgrade mechanism)
